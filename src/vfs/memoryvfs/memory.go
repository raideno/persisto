// SOURCE: "github.com/ncruces/go-sqlite3/vfs/memdb"
...
func RegisterMemoryVfs() {
	vfs.Register("memory", memVFS{})
}
...
func Create(name string, data []byte) {
	...
	sectors := divRoundUp(db.size, memorySectorSize)
	db.data = make([]*[memorySectorSize]byte, sectors)
	for i := range db.data {
		sector := data[i*memorySectorSize:]
		if len(sector) >= memorySectorSize {
			db.data[i] = (*[memorySectorSize]byte)(sector)
		} else {
			db.data[i] = new([memorySectorSize]byte)
...
func MemoryTestDB(tb testing.TB, params ...url.Values) string {
	tb.Helper()
...
const memorySectorSize = 65536

// Ensure memorySectorSize is a multiple of 64K (the largest page size).
var _ [0]struct{} = [memorySectorSize & 65535]struct{}{}
...
func (m *memFile) ReadAt(b []byte, off int64) (n int, err error) {
	...
	base := off / memorySectorSize
	rest := off % memorySectorSize
	have := int64(memorySectorSize)
	if base == int64(len(m.data))-1 {
		have = modRoundUp(m.size, memorySectorSize)
	}
...
func (m *memFile) WriteAt(b []byte, off int64) (n int, err error) {
	...
	for base >= int64(len(m.data)) {
		m.data = append(m.data, new([memorySectorSize]byte))
	}
...
func (m *memFile) truncate(size int64) error {
	if size < m.size {
		base := size / memorySectorSize
		rest := size % memorySectorSize
		if rest != 0 {
			clear((*m.data[base])[rest:])
		}
	}

	sectors := divRoundUp(size, memorySectorSize)
...
const memorySpinWait = 25 * time.Microsecond
...
func (m *memFile) Lock(lock vfs.LockLevel) error {
	...
	case vfs.LOCK_EXCLUSIVE:
		...
		for before := time.Now(); m.shared > 1; {
			if time.Since(before) > memorySpinWait {
				return sqlite3.BUSY
			}
			m.lockMtx.Unlock()
			runtime.Gosched()
...
func ListDatabases() []DatabaseInfo {
	memoryMtx.Lock()
	defer memoryMtx.Unlock()

	var databases []DatabaseInfo
	for name, db := range memoryDBs {
